...
public class ChatServer extends WebSocketServer {  
  ...
  public void handlePing(WebSocket conn, JSONObject jsonObject) {
	  ...
	  if (auth != null) {
		...
		try {
		    ...
		    PreparedStatement stmtGetPlayer = sqlConn.prepareStatement("SELECT * from players WHERE auth_token = ?");
		    ...
		    rs = stmt.executeQuery(getPlayerGameSql);
		    
		    // stmt.execute("UPDATE players SET last_seen = NOW(), ip_address = ? WHERE player_id = ?");
		}
  ...
  public void getPlayerFromAuth(String auth) {
