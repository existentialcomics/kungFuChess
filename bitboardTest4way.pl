...
if ($fen) {
    ...
    print "setup initial pos\n";
    KungFuChess::Bitboards::setupInitialPosition();
}
if ($frozenIn) {
    KungFuChess::Bitboards::setFrozen($frozenIn);
}
KungFuChess::Bitboards::resetAiBoards($color);
#KungFuChess::Bitboards::printBBSquares(); exit;
#print KungFuChess::Bitboards::pretty_ai();
#print KungFuChess::Bitboards::debug();
#print KungFuChess::Bitboards::debugNorth();
#print KungFuChess::Bitboards::debugSouth();
#print KungFuChess::Bitboards::debugEast();
#print KungFuChess::Bitboards::debugWest();
#exit;
...
while ($go) {
    ...
    if ($input =~ m/^[a-z][0-9][a-z][0-9]$/) {
        ...
        my ($eval, $moves, $material, $attacks) = KungFuChess::Bitboards::evaluate($color);
        ...
        print "\n";
        print "WHITE:\n";
        KungFuChess::BBHash::displayMoves($moves, 1, 0);
        print "BLACK:\n";
        KungFuChess::BBHash::displayMoves($moves, 2, 0);
